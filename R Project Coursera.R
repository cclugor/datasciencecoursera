...
# be one of ‚Äúheart attack‚Äù, ‚Äúheart failure‚Äù, or ‚Äúpneumonia‚Äù. Hospitals that do not have data on a particular
# outcome should be excluded from the set of hospitals when deciding the rankings.
# Handling ties. If there is a tie for the best hospital for a given outcome, then the hospital names should
# be sorted in alphabetical order and the first hospital in that set should be chosen (i.e. if hospitals ‚Äúb‚Äù, ‚Äúc‚Äù,
# and ‚Äúf‚Äù are tied for best, then hospital ‚Äúb‚Äù should be returned).
...
# function should throw an error via the stop function with the exact message ‚Äúinvalid state‚Äù. If an invalid
# outcome value is passed to best, the function should throw an error via the stop function with the exact
# message ‚Äúinvalid outcome‚Äù.
...
ggplotly(zz)
# incluir la tabla de muertes por millÛn de los diferentes paÌses 
```
...
highchart() %>%
  ...
  hc_subtitle(text = "PoblaciÛn por provincia",
              align = "center",
              style = list(color = "#0C5C9E", fontWeight = "bold")) %>% 
  hc_tooltip(followPointer =  FALSE) %>%
  hc_add_series_map(ecuador, poblacion_provincia_ecuador_2010, name = "PoblaciÛn",
